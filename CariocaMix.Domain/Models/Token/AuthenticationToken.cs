...
namespace CariocaMix.Domain.Models.Token
{
    public class AuthenticationToken
    {
        ...
        public static AdminAuthenticateModel GenerateTokenAdmin(Entities.Admin admin)
        {
            try
            {
                var tokenHandler = new JwtSecurityTokenHandler();
                ...
                var tokenDescriptor = new SecurityTokenDescriptor
                {
                    Subject = new ClaimsIdentity(new Claim[]
                    {
                        new Claim("idAdmin", admin.Id.ToString())
                    }),
                    Expires = DateTime.UtcNow.AddMonths(1),
                    SigningCredentials = new SigningCredentials(new SymmetricSecurityKey(key), SecurityAlgorithms.HmacSha256Signature)
                };
                ...
                AdminAuthenticateModel userAuthenticate = (AdminAuthenticateModel)admin;
                userAuthenticate.Token = tokenHandler.WriteToken(token);
                userAuthenticate.TokenExpires = tokenDescriptor.Expires;
        ...
        public static bool ValidateTokenAdmin(string token, long id)
        {
            try
            {
                var tokenHandler = new JwtSecurityTokenHandler();
                ...
                var jwtToken = (JwtSecurityToken)validatedToken;
                var accountId = long.Parse(jwtToken.Claims.First(x => x.Type == "idAdmin")?.Value);
